...
namespace TerrainEdit.DualContouring {
    public static class NormalSolver
    {
        ...
        public static void RecalculateNormals(this Mesh mesh, CubeGrid cube)
        {
            ...
            {
                var ia = mesh.triangles[tri];
                var ib= mesh.triangles[tri+1];
                var ic = mesh.triangles[tri+2];
                var a = mesh.vertices[ia];
                var b = mesh.vertices[ib];
                var c = mesh.vertices[ic];
                var norm = -Vector3.Cross((b - a).normalized, (c - a).normalized).normalized;
                
                normals[ia] = (normals[ia] + norm).normalized;
                normals[ib] = (normals[ib] + norm).normalized;
                normals[ic] = (normals[ic] + norm).normalized;
            }
